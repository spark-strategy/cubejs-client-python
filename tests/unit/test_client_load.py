"""End-to-end wiring smoke tests: cube()/CubeClient -> HttpTransport -> polling
loop -> ResultSet, against a mocked HTTP server."""
...
from cubejs_client import Query, cube, dim


@respx.mock
def test_load_returns_result_set_usable_as_dataframe():
    ...
    result_set = client.load(Query().measures("Orders.count").dimensions("Orders.status"))
...
@respx.mock
def test_load_accepts_plain_dict_query_and_dim_filter():
