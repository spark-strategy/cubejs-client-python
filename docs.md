...
- Both a synchronous (`CubeClient`) and an async (`AsyncCubeClient`) client, sharing one core; `ResultSet` covers `regularQuery`, `compareDateRangeQuery`, and `blendingQuery`, including `drill_down()` and custom (PostgreSQL-interval-style) time granularities. The clients also cover `subscribe()` (HTTP-polling long-poll) and the CubeSQL interface (`cube_sql()`/`cube_sql_stream()`). See @/src/cubejs_client/docs.md for the package's internal layering.
...
### Things to Know
- `subscribe()`/long-polling and `cubeSql()`/streaming are now implemented (see @/src/cubejs_client/client/docs.md): `subscribe()` is a degraded HTTP-polling long-poll (native cancellation, not JS's mutex supersession), and `cube_sql()`/`cube_sql_stream()` cover the CubeSQL interface. The client-side `format` module and WebSocket transport remain deferred by design (absent), not missing functionality.
