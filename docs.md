...
### Things to Know
- Later phases — `subscribe()`/long-polling, `cubeSql()`/streaming, the client-side `format` module, and WebSocket transport — are deferred by design, not missing functionality. They raise `NotImplementedError` or are simply absent.
