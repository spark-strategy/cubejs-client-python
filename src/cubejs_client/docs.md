...
- `errors.py` (`CubeError`, `RequestError`, `CubeSqlError`, `MutexChangedError`) and `models/` (`SqlQuery`, `ProgressResult`) are small, self-contained ports of `RequestError.ts`, `SqlQuery.ts`, and `ProgressResult.ts` used across the other layers — not broken out into their own docs.md. `CubeSqlError` is raised by `core/cubesql.py`'s parsers; `MutexChangedError` is a name-compatible stub that is intentionally never raised (this port uses native cancellation, not JS's mutex supersession — see @/src/cubejs_client/client/docs.md).
...
```
- `__init__.py` is the only place that wires all layers together: it re-exports the public surface — `CubeClient`, `AsyncCubeClient`, the `Subscription`/`AsyncSubscription` handles, `Meta`, `ResultSet`, `PivotConfig`, the query DSL, the error classes (`CubeError`, `RequestError`, `CubeSqlError`, `MutexChangedError`) — and — critically — imports `results.pandas_adapter`, whose import has the side effect of attaching `to_pandas()`/`.df` onto the `ResultSet` class. Nothing else imports `results.pandas_adapter`.
...
- Beyond `load()`, the clients expose `cube_sql()`/`cube_sql_stream()` (the CubeSQL interface, parsed by `core/cubesql.py`) and `subscribe()` (a degraded HTTP-polling long-poll returning a `Subscription`/`AsyncSubscription` handle). See @/src/cubejs_client/client/docs.md for the polling and cancellation model.
